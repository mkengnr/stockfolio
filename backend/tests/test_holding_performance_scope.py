...
def test_source_scope_with_sell_keeps_scope_separation():
    sg_a, sg_b = uuid.uuid4(), uuid.uuid4()
    lot_a, lot_b = uuid.uuid4(), uuid.uuid4()
    holding = _holding([
        _tx(tx_id=uuid.uuid4(), sgid=sg_a, type_=TransactionType.BUY, qty="10", price="100", day=1, lot_id=lot_a),
        _tx(tx_id=uuid.uuid4(), sgid=sg_b, type_=TransactionType.BUY, qty="5", price="200", day=2, lot_id=lot_b),
        _tx(
            tx_id=uuid.uuid4(), sgid=sg_a, type_=TransactionType.SELL, qty="4", price="120", day=3,
            allocations=[SimpleNamespace(buy_lot_id=lot_a, quantity=Decimal("4"))],
        ),
    ])
    sources = [
        SimpleNamespace(id=sg_a, name="A", color="#111111"),
    ...
    assert perf.remaining_cost_basis == Decimal("600")  # 6 remaining * 100
    assert perf.current_value == Decimal("900")          # 6 * 150
    assert [b.source_group_id for b in breakdown] == [sg_a]
    assert breakdown[0].remaining_quantity == Decimal("6")


def test_unclassified_scope_limits_to_none_source():
    sg_a = uuid.uuid4()
    lot_u, lot_a = uuid.uuid4(), uuid.uuid4()
    holding = _holding([
        _tx(tx_id=uuid.uuid4(), sgid=None, type_=TransactionType.BUY, qty="3", price="100", day=1, lot_id=lot_u),
        _tx(tx_id=uuid.uuid4(), sgid=sg_a, type_=TransactionType.BUY, qty="5", price="200", day=2, lot_id=lot_a),
    ])
    sources = [SimpleNamespace(id=sg_a, name="A", color="#111111")]
    perf, breakdown = _holding_performance(
        holding, Decimal("150"), sources, scope=PortfolioScope("unclassified")
    )
    assert perf.total_invested_principal == Decimal("300")  # only unclassified: 3*100
    assert perf.remaining_cost_basis == Decimal("300")
    assert [b.source_group_id for b in breakdown] == [None]
    assert breakdown[0].name == "미분류"
